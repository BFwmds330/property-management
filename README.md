...
- **公告**：顶部"📢 公告"页发布小区通知（可置顶），总览页自动显示最新三条；
- **预收款**：一键缴清时**多收的钱自动转成这户的预收余额**（不再拦着不让收）；房屋详情能看到余额和台账，也能手工登记预收；下次缴费现金不够时，在一键缴清页勾选"用预收余额抵扣"即可；
- **收据打印**：缴费流水页勾选若干笔 → 点"🖨 打印勾选的收据"，一联一张收据（金额大写+小写都有），浏览器打印或"另存为 PDF"都行；
- **报修拍照**：新增报修时可以附上现场照片（手机拍的 jpg/png 都行，5MB 以内），列表里点"📷 查看"。
...
1. 手机和电脑连**同一个 WiFi**
2. 双击 **`手机访问.bat`** 启动（代替 `启动.bat`）——启动窗口里会显示“手机访问：http://192.168.x.x:5000”这样的地址，并给出安全提醒
3. 手机浏览器打开该地址即可查看总览、住户、欠费（建议只读浏览，录入在电脑上做）

> ⚠️ 从 v2.2.0 起，普通双击 `启动.bat` **只允许本机访问**（更安全）。手机要看才用 `手机访问.bat`，或自行设置环境变量 `WUYE_LAN=1`。请只在可信的 WiFi 下开放。

### 启动密码（可选，默认关闭）

在局域网开放时，建议加一道 4 位数字密码：

- **推荐：在系统里直接设置**——「⚙️ 设置 → 🔐 启动密码」，输入两次 4 位数字点"启用"即可；以后可随时在同一页面修改或关闭（需先输当前密码）；
- 设置后，每次打开系统（或关闭浏览器后再打开）会先出现密码验证页，输入正确才能进入；
- 密码只以加密哈希保存在数据里，看不到原文——请牢记！忘记密码时，用"启用密码之前"的备份恢复即可进入；
- 备选：也可在命令行执行 `set WUYE_PIN=8848` 后启动（会话级密码，重启后失效）。
...
├── 启动.bat              # Windows 一键启动（仅本机访问）
├── 手机访问.bat           # 手机/平板访问模式（对同一 WiFi 开放）
├── 启动.command          # Mac 一键启动
└── README.md
```
