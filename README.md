...
- 关闭系统后，在“命令提示符”里进入本文件夹，执行 `set WUYE_PIN=1` 再运行 `手机访问.bat`——每次启动会**随机生成**一个 4 位密码，显示在启动窗口里；
- 或执行 `set WUYE_PIN=8848`（换成你想要的 4 位数字）——使用**固定密码**；
- 设置后，浏览器打开系统会先出现密码验证页，输入正确才能进入（本次使用期间不再询问）；
- 不设置 `WUYE_PIN` 就是没有密码，和以前一样。
